...
\begin{solution}
    ...
    \[\boxed{f(x)=x-\dfrac{1}{2}+\dfrac{1}{2}e^{-2x}}\]
\end{solution}
\begin{example}
...
\begin{solution}
    ...
    \end{align*}
    即通解为
    \[\boxed{y=\left[\dfrac{x^2(x^2-2)+C}{8(x^2-1)}\right]^2}\]
\end{solution}
\section{全微分方程}
...
\begin{theorem}[曲线积分法求解全微分方程]
    若$P(x,y),Q(x,y)$在某一单连通区域上连续且具有连续偏导数，且满足$\dfrac{\partial P}{\partial y}=\dfrac{\partial Q}{\partial x}$，则方程$P(x,y)\d x+Q(x,y)\d y=0$是全微分方程，且其通解为$u(x,y)=C$，其中
    \[ u(x,y)= \int_{(x_0,y_0)}^{(x,y)} P(x,y)\d x + Q(x,y)\d y \]
\end{theorem}

若对全微分有足够的熟练度，能利用常见的全微分公式进行配凑，则无需使用曲线积分法．
...
\begin{table}[H]
    ...
    \begin{tabular}{ll}
        ...
        $\dfrac{y \d x - x \d y}{xy}=\d\left(\ln\dfrac{x}{y}\right)$ &
        $\dfrac{y\d x-x\d y}{x^2-y^2}=\d\left(\ln\sqrt{\dfrac{x-y}{x+y}}\right)$ \\
...
\begin{solution}
    ...
    容易发现
    \[ \pdv{P}{y} = 2 = \pdv{Q}{x} \]
    故该方程是全微分方程，设其势函数为 $u(x,y)$，则
    \begin{align*}
        u(x,y) &= \int_{(x_0,y_0)}^{(x,y)} P(x,y)\d x + Q(x,y)\d y \\
        &= \int_{x_0}^x (3t^2+2y_0)\d t + \int_{y_0}^y (2x-3t^2)\d t \\
        &= x^3 + 2xy - y^3 + C
    \end{align*}
    于是该方程的通解为
    \[ \boxed{x^3 + 2xy - y^3 = C} \]
...
\begin{solution}
    ...
    即通解为
    \[\boxed{y=\dfrac{C_1}{x^2}+C_2},\,C_1 = -\dfrac{C_3}{2}\]
\end{solution}
...
\begin{theorem}[待定系数法]
    ...
    \begin{enumerate}
        ...
        \end{cases}
        \]
        \item 若$f(x)=e^{\alpha x}\left[P_l(x)\cos \beta x + Q_m(x)\sin \beta x\right]$，
        则可设非齐次特解
        \[y^*=x^k e^{\alpha x}\left[M_n(x)\cos \beta x + N_n(x)\sin \beta x\right]\]
        其中$n=\max{\{l,m\}}$，
        \[
        k = \begin{cases}
...
\begin{solution}
    ...
    故所求通解为
    \[\boxed{y=(C_1+C_2 x)e^{-2x}+\dfrac{1}{2}x^2 e^{-2x}}\]
\end{solution}
\begin{example}
...
\begin{solution}
    ...
    故所求通解为
    \[\boxed{y=e^{x}(C_1 \cos 2x + C_2 \sin 2x)+\dfrac{1}{8}xe^{x} \sin 2x}\]
\end{solution}
...
\begin{solution}
    ...
    \[\boxed{y=C_1 + C_2 x + e^{x}(C_3 \cos 2x + C_4 \sin 2x)}\]
\end{solution}
\section{欧拉方程}
\begin{definition}
    形如
...
\begin{solution}
    ...
    \[ \boxed{\left(\frac{1+x}{x}\right)^2y^2+(1+x^2)=C} \]
    法二：\\
    原方程化为
    \[
...
\begin{solution}
    ...
    \end{align*}
    故通解为
    \[\boxed{\sqrt{1-C_1^2y^2}-\ln\left|\dfrac{1+\sqrt{1-C_1^2y^2}}{C_1y}\right| = C_1x+C_2}\]
\end{solution}
...
\begin{solution}
    ...
    \[\boxed{C(y^2-2x^2-3)^3=(y^2 - x^2 -1)^2}\]
\end{solution}
...
\begin{solution}
    ...
    \[\boxed{y=\dfrac{1}{12}x^{\frac{3}{2}} - 4\sqrt{x} + \dfrac{32}{3}}\]
    $x=0$时，$y=\dfrac{32}{3}$，即导弹A、B相遇时的坐标为$\boxed{(0,\dfrac{32}{3})}$.
\end{solution}
