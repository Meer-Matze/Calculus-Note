...
\begin{corollary}
    ...
    \begin{itemize}
        ...
        \item 若$m,n$均为偶数，则：\begin{itemize}
...
\begin{solution}
    \begin{enumerate}[label=(\arabic*)]
        \item \[0\leq\left| \dfrac{xy}{\sqrt{x^2+y^2}} \right| \leq |x| \to 0 (x,y) \to (0,0)\]
        故原极限为$\boxed{0}$．
        \item \begin{align*}
        ...
        \end{align*}
        其中 $0 < \dfrac{x^2}{x^2+y^2} \leq 1$，且 $\lim\limits_{(x,y) \to (0,0)} y^4 = 0$，由 $0 \times$ 有界量法，原极限为$\boxed{0}$．\\
        \item 法一：
        \[ \lim\limits_{(x,y) \to (0,0)} \dfrac{x^2 +y^2}{|x| + |y|}= \lim\limits_{(x,y) \to (0,0)} |x| \cdot \dfrac{|x|}{|x| + |y|} + |y| \cdot \dfrac{|y|}{|x| + |y|} \]
        其中 $0 \leq \dfrac{|x|}{|x| + |y|} \leq 1$，$0 \leq \dfrac{|y|}{|x| + |y|} \leq 1$，且 $\lim\limits_{(x,y) \to (0,0)} |x| = 0$，$\lim\limits_{(x,y) \to (0,0)} |y| = 0$，由 $0 \times$ 有界量法，原极限为$\boxed{0}$．\\
...
\begin{solution}
    ...
    因为 $\lim\limits_{(x,y) \to (0,0)} f(x,y) = f(0,0) = 0$，故$\boxed{\text{函数 } f(x,y) \text{ 在点 } (0,0) \text{ 处连续}}$．
\end{solution}
\subsection{二次极限}
\begin{example}
...
\end{example}
\begin{solution}
    由于$\lim\limits_{y \to 0} x\sin\dfrac{1}{y}$ 不存在，故整个极限$\boxed{\text{不存在}}$．
\end{solution}
\ifx\allfiles\undefined
\end{document}
\fi